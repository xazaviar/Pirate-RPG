...
Game.prototype.addNewPlayer = function(socket, data) {
    ...
    var skillTrees = initializeSkillTrees(this.skillTreeList, this.skillList, this.comboList, this.buffList);
...
function initializeSkillTrees(treeList, skillList, comboList, buffList){
    ...
    for(var t in treeList){
        ...
        for(var tier = 0; tier < 5; tier++){
            ...
            for(var s in treeList[t].tiers[tier]){
                var skill = JSON.parse(JSON.stringify(skillList[treeList[t].tiers[tier][s].id]));
                ...
                if(skill.base.hasOwnProperty("buffs")){
                    var buff = [];
                    for(var b in skill.base.buffs){
                        buff.push(new Buff({
                            id: buffList[skill.base.buffs[b]].id,
                            name: buffList[skill.base.buffs[b]].name,
                            type: buffList[skill.base.buffs[b]].type,
                            stats: buffList[skill.base.buffs[b]].stats,
                            duration: buffList[skill.base.buffs[b]].duration                            
                        }));
                    }
                    skill.base.buffs = buff;
                }
