...
function Player(id, curMap, spawnCoords, instanceID, skillTrees) {
  	...
  	this.hpStaticMax	= 200;
  	this.hpMax 			= this.hpStaticMax;
  	this.hp 			= this.hpStaticMax;
  	this.energyStaticMax= 100;
  	this.energyMax 		= this.energyStaticMax;
  	this.energy 		= this.energyStaticMax;
  	...
  	//Activatables
  	this.activeList = [null,null,null,null,
  					   null,null,null,null,
  					   null,null,null,null];
  	this.activeStance = null;
  	this.activeOnCooldown = [];
  	this.curBuffList = [];
  	...
  	this.prevApply 		= {};
  	this.prevSwitch 	= -1;
  	this.prevAssign		= null;
  	this.prevMouseLeft 	= false;
  	this.prevMouseRight = false;
  	this.prevDodge 		= false;
  	this.prevActive		= []

  	for(var a in this.activeList) this.prevActive.push(false); //fill prevActive
...
Player.prototype.updateOnInput = function(keyboardState, mouseState, menuState) {
	if(this.isDead && keyboardState.respawn){
		...
		//Do menuing stuff
	  	...
	  	if(menuState.switchTree > -1 && this.prevSwitch == -1){
	  		...
	  		if(this.skillTrees[menuState.applySP.tree].increaseSkill(menuState.applySP.skill, this.skillTrees)){
	  			...
	  			//Auto place Skill
	  			var skillInfo = this.findSkill(menuState.applySP.tree, menuState.applySP.skill);
	  			// console.log(skillInfo);
	  			if(skillInfo.skill.type == "active"){
	  				for(var a in this.activeList){
	  					if(this.activeList[a] == null){
	  						this.activeList[a] = {
				  				type: "skill",
				  				tree: skillInfo.tree,
	  					...
	  					else if(this.activeList[a].skill.id == skillInfo.skill.id) break;
	  				}
	  			}
	  		}
	  	}
	  	else if(menuState.assignAction != null && this.prevAssign == null){
	  		if(menuState.assignAction.type == "skill"){
	  			var skillInfo = this.findSkill(menuState.assignAction.tree, menuState.assignAction.action);

	  			if(skillInfo.skill.level > 0 && skillInfo.skill.type == "active")
		  			this.activeList[menuState.assignAction.index] = {
		  				type: menuState.assignAction.type,
		  				tree: skillInfo.tree,	
		  				skill: skillInfo.skill
		  			};
	  		}
	  	...
	  	this.prevAssign = menuState.assignAction;
	}
		...
		//Check for blocking
	  	...
	  	for(var a in keyboardState.actives){
	  		if(keyboardState.actives[a] && !this.prevActive[a] && this.activeList[a] != null){
	  			this.useAbility(this.activeList[a]);
	  			break;
	  		}
	  	}
	  	...
	  	this.prevActive		= keyboardState.actives;
	}
	else{
		this.requestRespawn = false;
	}
...
Player.prototype.constUpdate = function() {
	...
	for(var a = 0; a < this.activeOnCooldown.length; a++){
		if(this.activeOnCooldown[a].onCooldown())
			this.activeOnCooldown[a].decrementCooldown();
		else{
			this.activeOnCooldown.splice(a,1);
			a--;
		}
	}

	//Decrease buff cooldowns
	for(var b = 0; b < this.curBuffList.length; b++){
		if(this.curBuffList[b].isActive())
			this.curBuffList[b].decrementDuration();
		else{
			this.curBuffList.splice(b,1);
			b--;
		}
	}
}
...
Player.prototype.useAbility = function(ability) {
	var activeTree = this.skillTrees[ability.tree].type != "CombatMain" || this.selectedCombatTree == ability.tree;

	//Check if skill is in active trees
	if(ability.type == "skill" && !ability.skill.onCooldown() && activeTree){
		var energyUsage = 0;
		if(ability.skill.curSkill.hasOwnProperty("energyUsage")){
			energyUsage = ability.skill.curSkill.energyUsage;
		}

		if(energyUsage <= this.energy){
			this.energy -= energyUsage;

			if(ability.skill.subtype == "stance"){
				this.applyNewStance(ability.skill);
			}
			else if(ability.skill.curSkill.hasOwnProperty("buffs")){
				//set on cooldown
				ability.skill.putOnCooldown();
				this.activeOnCooldown.push(ability.skill);
				
				for(var b in ability.skill.curSkill.buffs){
					ability.skill.curSkill.buffs[b].activate();
					this.curBuffList.push(ability.skill.curSkill.buffs[b]);
				}
			}
		}
	}
...
Player.prototype.findSkill = function(tree, id){
	var trees = this.skillTrees;

	for(var t in trees){
		var checkTree = trees[t];
		if(tree) checkTree = trees[tree];

		for(var tier in checkTree.tiers){
			for(var s in checkTree.tiers[tier]){
				if(checkTree.tiers[tier][s].id == id){
					return {
						tree: tree?tree:t,
						skill: checkTree.tiers[tier][s]
					};
				}
			} //Close Skills
		} //Close Tiers
...
Player.prototype.applyNewStance = function(stance){
	//Chekc if new stance
	if(this.activeStance == null || stance.id != this.activeStance.id){
		if(this.activeStance != null){
			stance.putOnCooldown();
			this.activeOnCooldown.push(stance);
		}
		...
		if(this.activeStance.curSkill.hasOwnProperty("energyMax"))
			this.energyMax += this.activeStance.curSkill.energyMax;
		if(this.activeStance.curSkill.hasOwnProperty("hpMax"))
			this.hpMax += this.activeStance.curSkill.hpMax;

		this.curBuffList.push(stance.curSkill.buffs[0]);
	}
	else{
		stance.putOnCooldown();
		this.activeOnCooldown.push(stance);

		for(var b in this.curBuffList){
			if(this.curBuffList[b].id == this.activeStance.curSkill.buffs[0].id){
				this.curBuffList.splice(b,1);
				break;
			}
		}
